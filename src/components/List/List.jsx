...
// is se pta chl rha left side mai pauch rha ky data 
  const{receivekrega,settype,type,ratekro,setrating}=useContext(GlobalContext)
  ...
  useEffect(()=>{
    ...
    const refs = Array(places?.length).fill().map((_, i) => startingref[i] ||createRef());
...
// ]
  return (
      ...
      <div className="listing" style={{padding:'25px',userSelect: 'none'}}>
        ...
        <span className='heading' style={{fontWeight:'bold',fontSize:'26px',userSelect: 'none'}}>
        ...
        <div  style={{  margin: '15px', minWidth: 120, marginBottom: '30px'}} className="box">
...
{/* filterimportant file mai maine yh explain kiya yhi chij  */}
         {/* important->  idhr ek confusion askta hm context hook mai ek child se dusre child mai interaction mai use krte hai but isme aisa ku nhi hua dhyan se dekho hmne kya kiya type hota ho receive krene vala mera short form aisa chlta and settype hota jo send kr rha toh isme do no ek mai hi hai type bhi isme hai settype bhi isme hai  */}
          <Select value={type} onChange={(e)=>settype(e.target.value)} style={{outline:'none'}}>
        ...
        <div  style={{ margin: '15px', minWidth: '120px', marginBottom: '30px',userSelect: 'none'}} className="box">
        <InputLabel >Rating</InputLabel>
          
          <Select  value={ratekro} onChange={(e)=>setrating(e.target.value)} >
      ...
      <Grid container spacing={3} className='griding' style={{userSelect: 'none'}}>
